...
const seededAnnotationDatasets = new Set<string>();
const annotationCountStmt = db.prepare(
  'SELECT count(*) AS n FROM annotations WHERE dataset_id = ?'
);

function ensureAnnotationsSeeded(datasetId: string): void {
  if (seededAnnotationDatasets.has(datasetId)) return;
  seededAnnotationDatasets.add(datasetId);
  const dir = datasetDir(datasetId);
  if (!dir) return;
  const file = path.join(dir, 'annotations.json');
  if (!existsSync(file)) return;
  if (Number((annotationCountStmt.get(datasetId) as { n: number }).n) > 0) return;
  let parsed: unknown;
  try {
...
type SeedDecision = Partial<Decision> & {
  id?: string;
  insight_title?: string;
  decision_type?: string;
  related_node_ids?: string[];
  insight_snapshot?: unknown;
};
...
function ensureDecisionsSeeded(datasetId: string): void {
  if (seededDecisionDatasets.has(datasetId)) return;
  seededDecisionDatasets.add(datasetId);
  const dir = datasetDir(datasetId);
  if (!dir) return;
  ...
  if (!Array.isArray(parsed)) return;
  for (const raw of parsed) {
    const d = raw as SeedDecision;
    const decisionType = d.decision_type;
    if (
      !d.id ||
      !d.insight_title ||
      !['accept', 'override', 'dismiss'].includes(String(decisionType))
    ) {
      continue;
    }
    seedDecisionStmt.run({
      id: String(d.id),
      dataset_id: datasetId,
      session_id: d.session_id ?? null,
      insight_card_id: d.insight_card_id ?? null,
      insight_title: String(d.insight_title),
      decision_type: String(decisionType),
      rationale: d.rationale ?? null,
      related_node_ids: JSON.stringify(d.related_node_ids ?? []),
      insight_snapshot: d.insight_snapshot
        ? JSON.stringify(d.insight_snapshot)
        : null,
...
export function getDecisions(filter: {
  ...
  ensureDecisionsSeeded(filter.datasetId);
