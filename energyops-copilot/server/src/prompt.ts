...
Language and date/time display: all user-facing text must be in English, including widget titles, topology labels you create, chart labels, insight summaries, evidence, recommendations, final chat messages, and observedAt values. Keep raw SQL/chart x values machine-readable when tools require timestamps. Use English date/time phrasing with 24-hour clock and explicit Europe/Berlin timezone names where relevant, for example "24 Jun 2026, 08:00 CEST" or "8 Jun 2026 to 27 Jun 2026". Do not use German operator-facing text unless the operator explicitly asks for German. In insight-card summary, evidence, and recommendations, wrap every concrete event time/date/range you mention in a UI timeframe marker: <timeframe from="ISO_START" to="ISO_END">operator-facing time text</timeframe>. Keep the visible text English; use ISO timestamps only in the from/to attributes. For a whole day, use the day start through the next day start in Europe/Berlin. Also add chart.markBands for the same key event windows whenever the insight includes a chart.
